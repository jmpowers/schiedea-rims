...
# plot(cophylo(as_phylo(ceb), sch.f))
```
...
pol.sppair %>% group_by(dataset, crossname, type) %>% 
  summarize(n.mompid = length(unique(mompid)), n.dadpid = length(unique(dadpid)), n.fullcross = length(unique(fullcross)), n.slides = n(), min.grains = min(tot), median.grains = median(tot),  max.grains = max(tot), n.grains = sum(tot)) %>% kable()
...
pf %>%  filter(crossname %in% sppairs) %>% 
  ...
  scale_color_brewer(palette = "Paired") + xlim(c(0,NA))+
