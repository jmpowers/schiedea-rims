---
title: "Hybrids Inflo Flowers GLMM"
author: "John Powers"
date: "Nov 2, 2017"
output:
  html_document:
...
#new tidyverse way that keeps inflo number:
ib <- ibold %>% group_by(crossid,plantid,cross,mompop,momid,species,dadpop,dadid,dadsp,crosstype,sxc,sxcxm,mompid,dadpid,smompop) %>% summarize(mass=sum(mass, na.rm=T), collect.date=max(collect.date, na.rm=T), inflo=sum(inflo, na.rm=T))
...
```{r fperi}
#get median and IQR of flowers per infloresence in the inflos picked for regression
ibold %>% group_by(sxc) %>% 
  summarize_at("flrs", list(~median(., na.rm=T), ~quantile(.,0.25,na.rm=T), ~quantile(.,0.75,na.rm=T)))

#or divide the total estimated flowers by the number of inflos - less biased
ib %>% mutate(flrs_per_inflo = round(flrs / inflo)) %>% group_by(sxc) %>% 
  summarize_at("flrs_per_inflo", list(~median(., na.rm=T), ~quantile(.,0.25,na.rm=T), ~quantile(.,0.75,na.rm=T)))

ib %>% group_by(sxc) %>%summarize_at("inflo", list(~median(., na.rm=T), ~quantile(.,0.25,na.rm=T), ~quantile(.,0.75,na.rm=T)))
```
...
```{r lsmeans, fig.height=5, fig.width=5.2}
#Count
