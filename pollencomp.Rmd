...
inventory.momid <- 
  seeds %>% count(momsp, type, dadsp, momid) %>% count(momsp, type, dadsp, name="crosses_seeds") %>% 
  full_join(germ %>% count(momsp, type, dadsp, momid) %>% count(momsp, type, dadsp, name="crosses_germ")) %>% 
  full_join(surv %>% count(momsp, type, dadsp, momid) %>% count(momsp, type, dadsp, name="crosses_surv"))
...
inventory.dadid %>% select(-dadsp) %>% 
  pivot_wider(names_from = type, values_from = starts_with("dadids"), names_vary = "slowest") %>% 
  rename_with(~str_remove(.x, "dadids_")) %>% #write_tsv("data/pollencomp_dadids.tsv") %>% 
  kable(caption = "Number of paternal combinations per maternal plant")
