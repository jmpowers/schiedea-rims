...
vigornames <- c(hybrid.vigor.germsurv="Hybrid/intraspecific fitness:\ngermination and survival", 
                ...
                prop.hybrid="Proportion of hybrid offspring", 
                prop.hybrid.sds = "Proporiton of hybrid seeds",
                prop.hybrid.zyg = "Proporiton of hybrid zygotes")
...
hybrid.test <- surv %>% filter(type=="mixed") %>% 
  ...
  mutate(hybrid.vigor.all = capsule.formed * sds * prop.germ * prop.alive,
         prop.hybrid.zyg = prop.hybrid / hybrid.vigor.all, #estimated number of fertilized zygotes that are hybrids
         hybrid.vigor.germsurv = prop.germ * prop.alive, 
         prop.hybrid.sds = prop.hybrid / hybrid.vigor.germsurv) #estimated number of planted seeds that are hybrids

hybrid.test %>% select(momsp, prop.hybrid, hybrid.vigor.germsurv, prop.hybrid.sds, hybrid.vigor.all, prop.hybrid.zyg) %>% 
  ...
  scale_x_discrete(labels=vigornames, position="top")+
  theme(axis.title = element_blank(), axis.text.y=element_text(color=sp_pal_dark, face="italic"))
```
...
hybrid.phylostyle <- left_join(hybrid.test, phylodist) %>% 
  ...
  pivot_longer(c(prop.hybrid, hybrid.vigor.germsurv, prop.hybrid.sds, hybrid.vigor.all, prop.hybrid.zyg)) 
...
hybrid.phylostyle %>% pivot_longer(c(pd, style_length_mm), names_to = "xname", values_to = "xvalue") %>% 
  ...
  facet_grid(name ~ xname, scales="free", switch="both", 
             labeller=as_labeller(c(vigornames, style_length_mm = "Style length (mm)", 
