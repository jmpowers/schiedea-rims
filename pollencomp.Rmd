...
```{r ranef}
comp.ranef <- map_dfr(comp.mod, ~as.data.frame(ranef(.x)), .id="trait") %>% 
  rename(momid=grp, estimate = condval, sd = condsd) %>% select(-component, -grpvar, -term) %>% 
  mutate(momid= as.character(momid))

ggplot(comp.ranef, aes(y=estimate, ymin=estimate-sd, ymax=estimate+sd, x=momid)) + facet_wrap(vars(trait)) +  geom_pointrange() + coord_flip()

# comp.ranef %>% select(grp, trait, condval) %>% pivot_wider(names_from="trait", values_from="condval") %>% 
#   ggplot(aes(x=prop.alive, y=prop.germ)) + geom_point() + geom_smooth()
```
