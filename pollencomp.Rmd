...
alternate <- geom_tile(data=tibble(momsp=sp_pairs[seq(1,10,by=2)]), aes(x=momsp, y=0.5, height=1, width=1),  alpha=0.2, fill="grey", inherit.aes = F) 

ggplot(seeds.bycross, aes(x=momsp, fill=type, y=capsule.formed))  +
  geom_boxplot(position = position_dodge(width=0.9), color="grey50", outlier.size=0.5) + 
  geom_pointrange(data=filter(comp.emm, trait=="capsule.formed"), aes(y=estimate, ymax=conf.high, ymin=conf.low),
...
ggplot(seeds %>% filter(sds>0), aes(x=momsp, fill=type, y=sds))  +
  geom_boxplot(position = position_dodge(width=0.9), color="grey50", outlier.size=0.5) + 
  geom_pointrange(data=filter(comp.emm, trait=="sds"), aes(y=estimate, ymax=conf.high, ymin=conf.low),
                  position = position_dodge(width=0.9), size=0.3, show.legend=F) + 
  scale_fill_manual(values=type_pal, labels=typenames) + 
