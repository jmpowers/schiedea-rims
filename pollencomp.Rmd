...
left_join(postzygotic.ratios, phylodist) %>% 
  ...
  geom_point(aes(color=momsp), size=2) + 
...
```{r phylo_arc, fig.width=4, fig.height=4}
par(mfrow=c(1,2))
par(mar=c(1.5,0,1.5,0))
plot(tree_pairs, tip.color = sp_pal_dark[tree_pairs$tip.label])
par(mar=c(0,0,0,0))
sp_pairs %>% matrix(ncol=2, byrow=T) %>% 
  arcplot(show.labels=F, horizontal=F, ordering=tree_pairs$tip.label, 
          col.arcs=sp_pal[(1:5)*2], col.nodes = sp_pal)
...
ggplot(left_join(hybrid.phylodist, filter(hybrid.phylodist.test, term=="pd")) %>% 
         filter(trait=="prop.hybrid", type=="mixed") %>% left_join(hybrid.test),
       aes(x=pd, y=estimate)) + coord_cartesian(xlim=c(0,NA))+
