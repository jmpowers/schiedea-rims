...
mod.pd.backest %>% select(-data, -model, -coef) %>% unnest(anova) %>% filter(term !="Residuals") %>%
  ungroup() %>% mutate(slope.pd = map_dbl(mod.pd.backest$coef, ~.x$cond[2])) %>% kable()

mod.pd.backest %>% select(-model, -coef, -anova) %>% unnest(data) %>% 
  ggplot(aes(x=pd, y=prop)) + 
  facet_wrap(vars(stage), 
             labeller = as_labeller(set_names(labs.phylostyle[2:5], c("plt", "sdl","sds","zyg")))) + 
  geom_point(aes(color=momsp)) + 
