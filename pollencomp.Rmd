...
ggplot(aes(x=prop.zero, y=sxc)) + 
  ...
  labs(x=paste0("Nonzero F1 flowers resulting from one P x P pollination (",ntrials," trials)"), y="Crosstype",
       title = "Frequency cumulativefitness greater than zero") 
```
...
```{r reg_pd, fig.width=7, fig.height=5}
...
pdseq <- tibble(pd=seq(min(phylodist$pd),max(phylodist$pd)+0.001, by=0.001), alive=1)#for predictions later
...
mod.pd.backest <- F1.backest.long %>% filter(samp==1) %>%  #just one sample
  ...
  group_by(stage) %>% nest() %>% 
  mutate(model = map(data, ~glmmTMB(prop.round ~ pd + (1|momsp), family = "betabinomial", weights=alive, data=.x)),
         coef = map(model, ~fixef(.x)),
         anova = map(model, ~tidy(car::Anova(.x))),
         pred = map(model, ~bind_cols(pdseq, predict(.x, newdata=pdseq, 
                                                       re.form=NA, type="response", se.fit=T))))
mod.pd.backest %>% select(stage, data) %>% unnest(data) %>% 
  ...
  geom_ribbon(data = unnest(mod.pd.backest, pred), 
              aes(ymax=fit+se.fit, ymin=fit-se.fit), fill="black", alpha=0.1)+
  geom_line(data = unnest(mod.pd.backest, pred), aes(y=fit)) +
  geom_jitter(aes(color=momsp, y=prop, size=alive), width=0.001, height=0) + scale_size(range=c(0,3))+
  ...
  scale_color_brewer("Maternal\nspecies", palette="Paired") + 
  labs(x=labs.phylostyle["pd"], y="")+ 
...
mod.pd.backest %>% select(stage, anova) %>% unnest(anova) %>% filter(term !="Residuals") %>%
  ungroup() %>% mutate(slope.pd = map_dbl(mod.pd.backest$coef, ~.x$cond[2])) %>% kable()
```
