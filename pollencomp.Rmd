...
```{r inference, fig.width=10, fig.height=3.5}
...
hybrid.test <- surv %>% filter(type=="mixed") %>% 
  ...
  mutate(hybrid.vigor.all = capsule.formed * sds * prop.germ * prop.alive, #these are all inter/intra relative fitnesses (hybrid vigor)
         ...
         prop.hybrid.cap = goback(prop.hybrid.sds, sds), #capsule stage
         prop.hybrid.zyg = goback(prop.hybrid.cap, capsule.formed)) #zygote stage

propnames <- c(prop.hybrid =     "Hybrid plants", 
               prop.hybrid.sdl = "Hybrid seedlings",
               prop.hybrid.sds = "Hybrid seeds",
               prop.hybrid.cap = "Hybrid capsules",#term is not accurate
               prop.hybrid.zyg = "Hybrid zygotes")
...
hybrid.test %>% select(momsp, all_of(names(propnames))) %>% 
  ...
  scale_color_brewer("Maternal species", palette="Paired") + scale_x_discrete(labels=propnames, position="top")+
  theme(axis.title = element_blank(), legend.text=element_text(face="italic")) 
#TODO colors and labels don't match
```
...
```{r fertilized, fig.width=6.5, fig.height=12}
hybrid.phylostyle <- left_join(hybrid.test, phylodist) %>% 
  left_join(select(styles.emm, momsp, style_length_mm, style_diff_rel)) %>% 
  pivot_longer(c(prop.hybrid, prop.hybrid.sds, hybrid.vigor.all, prop.hybrid.zyg)) 
...
hybrid.phylostyle %>% pivot_longer(c(pd, style_length_mm), names_to = "xname", values_to = "xvalue") %>% 
  ...
  facet_grid(name ~ xname, scales="free", switch="both", 
             labeller=as_labeller(c(vigornames, propnames, style_length_mm = "Style length (mm)", 
                                    pd="Phylogenetic distance (mutations/site)"))) + 
  geom_hline(aes(yintercept=if_else(name=="hybrid.vigor.germsurv",1,0.5)), linetype=2)+
  ...
  scale_y_continuous(expand=expansion(mult=c(0,0.05)), breaks=scales::breaks_extended(n=5)) + 
