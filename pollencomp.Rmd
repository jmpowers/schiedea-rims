...
postzygotic.ratios %>% left_join(comp.tests) %>% left_join(phylodist) %>% 
...
postzygotic.ratios %>% left_join(comp.tests) %>% left_join(phylodist) %>% 
  arrange(trait, pd) %>% select(trait, momsp, dadsp, RI, adj.p.value) %>% 
...
left_join(hybrid.test, phylodist) %>% 
  ...
  pivot_longer(cols=c(prop.hybrid, hybrid.vigor.germsurv, prop.hybrid.sds)) %>% 
  ggplot(aes(x=pd, y=value)) + facet_wrap(vars(name), scales="free_y", labeller=as_labeller(vigornames)) + expand_limits(y=c(1,0))+
  geom_hline(aes(yintercept=rep(c(0.5,1,0.5), length(sp_pairs))), linetype=2)+  scale_y_continuous(expand=expansion(mult=c(0,0)))+
  geom_smooth(method="lm", se=F, color="grey") +
  geom_point(aes(color=momsp), size=4) + scale_color_brewer(palette="Paired") + 
...
left_join(hybrid.test, select(styles.emm, momsp, style_length_mm, mating)) %>% 
  ...
  facet_wrap(vars(name), scales="free_y", labeller=as_labeller(vigornames)) + expand_limits(y=c(1,0))+
  geom_hline(aes(yintercept=rep(c(0.5,1,0.5), length(sp_pairs))), linetype=2)+  scale_y_continuous(expand=expansion(mult=c(0,0)))+
  geom_line(aes(group=pair, color=momsp))+ geom_point(aes(color=momsp), size=4) + scale_color_brewer(palette="Paired") + 
