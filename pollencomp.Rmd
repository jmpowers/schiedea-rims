...
sp_pairs <- c("menz"="menziesii", "sal"="salicaria",
              ...
              "memb"="membranacea","napa"="napaliensis", 
              "kaal"="kaalae", "hook"="hookeri")
#changed S. kauaiensis > S. napaliensis per Wagner & Weller 2025, doi:10.3897/phytokeys.254.148438
napalize <- function(x) x %>% str_replace("kauaiensis", "napaliensis") %>% str_replace("kaua","napa")
...
momid.fixes <- read_tsv("data/Pollen competition - momid_fixes2.tsv") %>% 
  drop_na(momid_fixed) %>% select(momid, momid_fixed) %>% deframe()

seeds <- read_tsv("data/Pollen competition - seeds.tsv") %>% 
  mutate(across(matches("sp"), napalize)) %>% 
  ...
  mutate(across(matches("sp"), ~ recode(.x, !!!sp_pairs)),
         ...
         across(matches("(dad|mom)id"), ~recode(.x, !!!momid.fixes)),
...
germ.pot <- read_tsv("data/Pollen competition - germination.tsv") %>%  
  ...
  mutate(across(matches("sp"), ~ recode(.x, !!!sp_pairs)),
         across(matches("id"), ~str_remove_all(.x, "[ #-]")), 
         across(matches("(dad|mom)id"), ~recode(.x, !!!momid.fixes)), #TODO are the momid fixes applicable to dads too?
         sxc = paste(momsp, type, sep="."),
         type=factor(type, levels= names(typenames)), 
         across(matches("sp"), ~ factor(.x, levels = sp_pairs)),
...
surv <- read_tsv("data/Pollen competition - survival.tsv") %>% 
  ...
  mutate(across(matches("(dad|mom)id"), ~str_remove_all(.x, "[ #-]")), 
         across(matches("(dad|mom)id"), ~recode(.x, !!!momid.fixes)),
         sxc = paste(momsp, type, sep="."),
         type=factor(type, levels= names(typenames)),
         across(matches("sp"), ~ factor(.x, levels = sp_pairs)),
...
# to run this, comment out dad|mom recode momid_fixes lines above
# count(seeds, momsp, momid, name="seed") %>% 
#   full_join(count(germ, momsp, momid, name="germ")) %>% 
#   full_join(count(surv, momsp, momid, name="surv")) %>%
#   arrange(momsp, momid) %>% 
#   write_tsv("data/seedsgermsurv_momids.tsv", na="") #then hand annotate to make momid_fixes2
...
```{r inventory}
list(seeds %>% summarize(across(sds, sum)),
  germ %>% summarize(across(c(planted, germinated), sum)),
  surv %>% summarize(across(c(scored=total, alive, hybrid), sum))) %>% reduce(bind_cols) %>% 
  kable(caption="Number of seeds counted, planted, germinated, scored, survived, and that were hybrids")

make.inventory <- function(x, name) {
  list(x %>% count(momsp, type, momid) %>% count(momsp, type, name=paste0("moms_",name)),
       x %>% count(momsp, type, momid, dadid) %>% count(momsp, type, name=paste0("momxdads_",name)),
       x %>% count(momsp, type, name=paste0("pollinations_",name))) %>% 
    reduce(full_join)
}
list(seeds, germ, surv) %>% #could also do germ.pot and do pots_per_cross = pots_germ/crosses_germ 
  map2(c("seeds","germ", "surv"), make.inventory) %>% reduce(full_join) %>% 
  bind_rows(summarise(.,across(where(is.numeric), sum, na.rm=T))) %>% 
  kable(caption ="Number of maternal plants, maternal x paternal combinations, and pollinations that made it into each dataset")

inventory.dadid <- seeds %>% count(momsp, type, dadsp, momid, dadidm, dadspm, dadsp, dadid) %>% 
  count(momsp, type, dadsp, momid, name="dadids_seeds") %>% 
  full_join(germ %>% count(momsp, type, dadsp, momid, dadidm, dadspm, dadsp, dadid) %>% 
              count(momsp, type, dadsp, momid, name="dadids_germ")) %>% 
  full_join(surv %>% count(momsp, type, dadsp, momid, dadidm, dadspm, dadsp, dadid) %>% 
              count(momsp, type, dadsp, momid, name="dadids_surv")) %>% 
  arrange(momsp, momid, type) %>% 
  select(-dadsp) %>% pivot_wider(names_from = type, values_from = starts_with("dadids"), names_vary = "slowest") %>%
  rename_with(~str_remove(.x, "dadids_")) #%>% write_tsv("data/pollencomp_dadids.tsv")
```
...
F1.cumul.fitness %>% group_by(sxc) %>% summarize(prop.zero=mean(fitness_sds>0)) %>% 
ggplot(aes(x=prop.zero, y=sxc)) + 
  ...
  labs(x=paste0("Nonzero F1 flowers resulting from one P x P pollination (",ntrials," trials)"), y="Crosstype",
       title = "Frequency cumulative fitness greater than zero") 
```
...
# Distribution of estimated hybrid proportions

```{r backwards, fig.height=10, fig.width=8}
...
ggplot(F1.backest.long, aes(x=fct_rev(stage), color=momsp, y=prop)) + 
  ...
  labs(x="Life stage", y="Proportion of hybrids")
```
...
comp.emm <-  map_dfr(comp.mod, ~emmeans(., ~sxc) %>% summary(type="response", level=1-2*pnorm(1, lower.tail=F)) %>%  #average over collect date #use a confidence level of 68% to get the standard error (z*=1)
...
ggplot(postzygotic.ratios, aes(y=fct_rev(momsp), x=trait, fill=log2(hybrid.vigor))) +
  ...
  theme(axis.title = element_blank(), axis.text.y=element_text(color=rev(sp_pal_dark)))
...
ggplot(filter(surv, type=="mixed"), aes(x=momsp, y=prop.hybrid, fill=momsp)) + 
  ...
  geom_pointrange(data=left_join(filter(comp.emm, trait=="prop.hybrid"), hybrid.test),
                  aes(y=estimate, ymax=conf.high, ymin=conf.low, shape=p.adj < 0.05), size=1) + #NB these are SEs
...
styles %>% count(momsp, plantid) %>% count(momsp, name="moms") %>% 
  left_join(count(styles, momsp, name="styles")) %>% mutate(styles_per_mom=styles/moms) %>% 
  kable(caption = "Number of plants and styles measured")
...
ggplot(styles, aes(x=momsp, y=style_length_mm, fill=momsp)) + 
  ...
  geom_pointrange(data=styles.emm, aes(ymax=style_length_mm+std.error, ymin=style_length_mm-std.error)) + 
...
postzygotic.all <- hybrid.test %>% select(momsp, hybrid.vigor=hybrid.vigor.all) %>% 
  mutate(trait="all", RI = 1-2*hybrid.vigor/(hybrid.vigor+1))

postzygotic.ratios.phylo <- postzygotic.ratios %>% bind_rows(postzygotic.all) %>% 
  mutate(trait = factor(trait, levels = c(names(propnames), "all"))) %>% 
  left_join(comp.tests) %>% left_join(phylodist) %>% 
  mutate(signif = adj.p.value < 0.05) %>% #TODO no p-values defined for cumul RI
...
ggplot(postzygotic.ratios.phylo, aes(x=pd, y=RI)) + 
  facet_wrap(vars(trait), labeller=as_labeller(c(propnames, all="Cumulative")), nrow=1)+ 
  geom_hline(yintercept=0, linetype=2) + geom_hline(yintercept=-1)+ geom_hline(yintercept=1)+
  geom_smooth(method="lm", se=F, color="grey") + 
  geom_errorbar(aes(color=momsp, ymin=low, ymax=high), width=0.001, linewidth=1) + #NB these are SEs
  geom_point(aes(color=momsp, shape = signif), size=2, stroke=2) + 
  scale_color_brewer(palette="Paired") +  scale_shape_manual(values=c(1, 19),guide="none", na.value=15)+
...
text(x=c(0.003, 0.008, 0.005), y=c(9, 4, 1)+0.4, c("Schiedea","Mononeura", "Alphaschiedea"), 
     cex=1.2, col="grey40")
...
```{r fertilized, fig.width=6.6, fig.height=12}
F1.backest.quantiles <- F1.backest.long %>% group_by(momsp, stage) %>% 
  summarize(q25 = quantile(prop, 0.25), 
            q50 = quantile(prop, 0.50), 
            q75 = quantile(prop, 0.75)) %>% 
  mutate(stage = paste0("prop.hybrid.", stage)) %>% 
  mutate(stage = recode(stage, "prop.hybrid.plt" = "prop.hybrid")) %>%
  rename(name = stage)
...
hybrid.phylostyle <- left_join(hybrid.test, phylodist) %>% 
  ...
  pivot_longer(all_of(c(names(hybnames), "hybrid.RI.all")) ) %>% 
  left_join(F1.backest.quantiles)

labs.phylostyle <- c(hybrid.RI.all="Reproductive isolation",#\n(heterosis \U2194 complete isolation)
                     prop.hybrid="Hybrid plants\n(observed)",
                     prop.hybrid.sdl="Hybrid seedlings\n(estimated)",
                     prop.hybrid.sds="Hybrid seeds\n(estimated)",
                     prop.hybrid.zyg = "Hybrid pollen\ncompetiveness (est.)",
...
hybrid.phylostyle %>% pivot_longer(c(pd, style_length_mm), names_to = "xname", values_to = "xvalue") %>% 
  ...
  facet_grid(name ~ xname, scales="free", switch="both", labeller=as_labeller(labs.phylostyle)) + 
  ...
  geom_errorbar(aes(color=momsp, ymin=q25, ymax=q75, width=if_else(xname == "pd", 0.001, 0.25))) + 
...
```{r reg_pd, fig.width=7, fig.height=7}
#regressions with one point per species
...
#regressions with sample size of the hybrid scoring
stylenames <- c("shorter", "longer")
pdseq <- expand_grid(pd=seq(min(phylodist$pd),max(phylodist$pd)+0.001, by=0.001),  #for predictions later
                     nesting(style_diff = factor(stylenames, levels= stylenames), 
                             style_diff_rel = seq(0, max(styles.emm$style_diff_rel), length.out=2)), alive=1)
stagenames <- rev(c("plt", "sdl","sds","zyg"))

mod.pd.backest <- F1.backest.long %>% filter(samp==1) %>%  #just one sample
  left_join(surv %>% filter(type=="mixed") %>% select(momsp, alive) %>% mutate(batch=row_number())) %>%
  mutate(hybrid = round(prop*alive), #give the estimates the same number of trials, and round successes
         prop.round=hybrid/alive) %>% 
  left_join(select(styles.emm, momsp, style_length_mm, style_diff_rel)) %>% 
  left_join(phylodist) %>% 
  mutate(stage = factor(stage, levels=stagenames)) %>% 
  group_by(stage) %>% nest() %>% 
  mutate(model = map(data, ~glmmTMB(prop.round ~ pd + style_diff_rel + (1|momsp), family = "betabinomial", weights=alive, data=.x)),
         coef = map(model, ~tidy(.x)),
         anova = map(model, ~tidy(car::Anova(.x))),
         pred = map(model, ~bind_cols(pdseq, predict(.x, newdata=pdseq, re.form=NA, type="response", se.fit=T))))
mod.pd.backest %>% select(stage, data) %>% unnest(data) %>% 
  ggplot(aes(x=pd)) + 
  facet_wrap(vars(stage), nrow=1,
             labeller = as_labeller(set_names(labs.phylostyle[2:5], rev(stagenames)))) + 
  geom_ribbon(data = unnest(mod.pd.backest, pred), 
              aes(ymax=fit+se.fit, ymin=fit-se.fit, group=style_diff), alpha=0.1)+
  geom_line(data = unnest(mod.pd.backest, pred), aes(y=fit, linetype=style_diff)) +
  geom_jitter(aes(color=momsp, y=prop, size=alive), width=0.001, height=0) +
  scale_size("Plants scored", breaks = c(10, 100), range=c(0,3), guide=guide_legend(order=1))+
  # geom_point(data = hybrid.phylostyle %>% mutate(stage=str_remove(name, "prop.hybrid.") %>%
  #                            str_replace("prop.hybrid", "plt") %>% factor(levels=stagenames)) %>%
  #              drop_na(stage),
  #            aes(color=momsp, y=value), size=4)+
  scale_color_brewer("Maternal\nspecies", palette="Paired", guide=guide_legend(override.aes = list(size = 4))) + 
  labs(x=labs.phylostyle["pd"], y="Proportion of hybrids")+ 
  scale_y_continuous(limits=c(0,1), expand = expansion(mult=c(0,0)))+
  scale_linetype_manual("Maternal style length (within the pair)", values=1:3, guide=guide_legend(order=2))+
  theme_bw() +
  theme(axis.text=element_text(color="black"), legend.position="top", legend.box = "vertical",
        panel.grid.minor = element_blank(), panel.grid.major.x=element_blank(), 
        panel.spacing = unit(1, "lines")) 

mod.pd.backest %>% select(stage, coef) %>% unnest(coef) %>% filter(term %in% c("pd", "style_diff_rel")) %>% 
  select(stage, term, p.value) %>% pivot_wider(names_from=term, values_from=p.value) %>% kable(caption="P values")
```
