...
tubes.mod.long %>% select(var, cont) %>% unnest(cont) %>% select(var, lvl1, lvl2, adj.p.value) %>%
  ...
  kable(caption = "Decline in pollen tubes from intraspecific to interspecific crosses", digits=2)
```
...
ggplot(tubes.phylodist, aes(x=pd, y=trait_diff)) + facet_grid(trait~hr, scales="free_y") +
  ...
  geom_pointrange(aes(color=momsp, ymin=trait_diff-trait_se, ymax=trait_diff+trait_se)) +
