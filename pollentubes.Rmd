...
* **hr** - the time elapsed since pollination: 4 h or 24 h
...
#wide format: memnap (3-5 stigmas/vial), speste (6 stigmas/vial), kaahoo_extra(3-5 stigmas/vial)
pops <- list(sali="842", menz = c("849", "950"), 
             lydg="6682", sarm="896", 
             sper= c("120289","120294","120296"), stel = c("14651","14765"),
             memb = c("mem","864"), napa = "napa",
...
pair_labels <- c(salmen = "S. salicaria | S. menziesii",
                 ...
                 memnap = "S. membranacea | S. napaliensis",
...
# membranacea - napaliensis
tubesl[["memnap"]] <- read_csv("data/pollentubes_memkau.csv") %>%
  mutate(across(matches("sp"), napalize)) %>% 
  mutate(across(ends_with("sp"), as.factor)) %>% 
  pivot_longer(starts_with(c("mid","bottom")), names_to=c("location","stigma"), names_sep="_", values_to="n_tubes") %>% 
  pivot_wider(names_from="location", values_from="n_tubes") %>% 
  ...
  drop_na(mid) %>% 
  mutate(momcross = ifelse(momsp %in% pops[["memb"]], "M", "N"),  dadcross = ifelse(dadsp %in% pops[["memb"]], "M", "N"),
...
tubes <- bind_rows(tubesl, .id = "pair") %>% 
  mutate(hr = factor(paste0(hr,"hr"), levels=c("4hr","24hr")),
         ...
         pair = factor(pair, levels=names(pair_labels)),
         momsp = species_names[momcross], dadsp=species_names[dadcross],
         type = factor(if_else(momcross==dadcross, "intra", "inter"))) %>% arrange(pair)
str(tubes)
```
...
list(tubes %>% count(momsp, type, momid) %>% count(momsp, type, name="moms"),
     tubes %>% count(momsp, type, momid, dadid) %>% count(momsp, type, name="momxdads"),
     tubes %>% count(momsp, type, hr, momid, vial) %>% count(momsp, type, name="pollinations"),
     tubes %>% count(momsp, type, name="styles")) %>% 
  reduce(full_join) %>% 
  bind_rows(summarise(.,across(where(is.numeric), sum))) %>% 
  kable(caption="Number of maternal plants, maternal x paternal combinations, pollinations, and styles 
        for each maternal species and cross type")
```
...
timefills <-   scale_fill_brewer("Time", palette="Paired", labels=c("4 h","24 h"))
timecolors <-   scale_color_brewer("Time", palette="Paired", labels=c("4 h","24 h"))
...
var_labels_letter <- paste(c("A.", "B.", "C."), var_labels) %>% set_names(names(var_labels))
...
tubes.phylodist %>% filter(trait=="ratio") %>% 
  ggplot(aes(x=pd, y=-trait_diff)) + facet_wrap(vars(hr), labeller = as_labeller(c(`4hr`="4 h", `24hr`="24 h"))) + 
