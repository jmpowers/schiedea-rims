...
```{r pollentubes, fig.width=12, fig.height=10}
mytheme <- theme(legend.text=element_text(size=rel(1)), legend.position="right", 
                 ...
                 axis.ticks.x = element_blank(), plot.title = element_text(face="italic"))
...
plot_emm <- function(pair, emm, cont, data, yvar) {
  ggplot(emm, aes(y=response, x=cross, fill=hr)) +
    geom_violin(data=data, aes_string(y=yvar),position=position_dodge(), 
                draw_quantiles=c(0.25,0.5,0.75), size=0.5, bw=ifelse(yvar=="ratio",0.1,1)) +
    ...
    geom_signif(data=emm, y_position=cont$height/c(mid=1, bottom=1, ratio=15)[yvar]+c(mid=19, bottom=19, ratio=1)[yvar], 
                xmin = cont$x1, xmax = cont$x2, annotations=cont$stars, vjust=0.5, tip_length=0.01) +
...
ratio.phylodist <- tubes.mod %>% select(pair, emm=ratio_emm) %>% unnest(emm) %>% 
  mutate(momsp = factor(species_names[str_sub(cross, 1,2)], levels=species_names)) %>% group_by(pair, momsp, hr) %>% 
  summarize(ratio_diff = response[1]-response[2], 
...
ggplot(ratio.phylodist, aes(x=pd, y=ratio_diff)) + facet_wrap(vars(hr)) +
  ...
  labs(x="Phylogenetic distance (mutations/site)", color="Maternal species",
       y="Difference between intra and interspecies cross in bottom:mid ratio") + theme_bw() + 
