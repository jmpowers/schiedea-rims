...
tubes.mod.plot.signif <- tubes.mod.plot %>% select(pair, hr, response, contrast, cross_pos) %>%  
  ...
  pivot_wider(names_from=startend, values_from=c(response, cross_pos)) %>% arrange(pair) %>% 
...
ggplot(tubes.mod.plot) +
  ...
  scale_color_brewer(palette="Set2") + scale_linetype_manual(values=c(3,1))+  guides(linetype="none", color="none")
}
plot_all_simple("mid")
...
ggplot(tubes.phylodist, aes(x=pd, y=trait_diff)) + facet_grid(trait~hr, scales="free_y") +
  geom_smooth(method="lm", se=F, color="grey") + geom_point(aes(color=momsp), size=2) +
...
tubes.phylodist %>% filter(trait=="ratio", hr=="24hr") %>% 
  ggplot(aes(x=pd, y=trait_diff)) + 
  geom_smooth(method="lm", se=F, color="grey") + geom_point(aes(color=momsp), size=3) + 
  scale_color_brewer(palette="Paired")+
  ...
  labs(x="Phylogenetic distance (mutations/site)", color="Maternal\nspecies", 
       y="Decrease in base/middle in hybrid crosses at 24 hr") + 
