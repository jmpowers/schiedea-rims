...
F1.traits <- traits %>% filter(generation=="F1",yvar!="viable.per.anther") %>% select(data, yvar, ylab, type)
...
ggplot(inflobiomass, aes(x=inflo.biomass.g, y=flrs/inflo.biomass.g, color=cross)) + 
  ...
  geom_smooth(data=inflobiomass.sum %>% filter(inflo>0, flrs>0), aes(x=inflo.biomass.g, y=flower.number/inflo.biomass.g), se=F)+ 
  scale_x_log10() + scale_y_log10() + 
  scale_color_manual(values=set_names(F1.col, c("KKwithin","KK", "KH", "HK", "HH", "HHwithin"))[-c(1,6)]) +
...
ggplot(inflobiomass.sum %>% filter(inflo>0, flrs>0), aes(x=inflo, y=flrs, color=cross)) + 
  ...
  scale_x_log10() + scale_y_log10() + 
  scale_color_manual(values=set_names(F1.col, c("KKwithin","KK", "KH", "HK", "HH", "HHwithin"))[-c(1,6)]) +
  theme_minimal() + labs(y="Flowers on one inflorescence", x="Total inflorescences", color="Cross")

ggplot(inflobiomass.sum %>% filter(inflo>0, flrs>0), aes(x=inflo, y=inflo.biomass.g/inflo, color=cross)) + 
  geom_point(size=0.5) +  geom_smooth(se=F, span=0.8) +
  scale_x_log10() + scale_y_log10() + 
  scale_color_manual(values=set_names(F1.col, c("KKwithin","KK", "KH", "HK", "HH", "HHwithin"))[-c(1,6)]) +
  theme_minimal() + labs(y="Average biomass per inflorescence (g)", x="Total inflorescences", color="Cross")

ggplot(inflobiomass.sum %>% filter(inflo>0, flrs>0), aes(x=inflo.biomass.g, y=flrs.per.inflo.biomass.g, color=cross)) + 
  geom_point(size=0.5) +  geom_smooth(se=F, span=0.8) +
  scale_x_log10() + scale_y_log10() + 
  scale_color_manual(values=set_names(F1.col, c("KKwithin","KK", "KH", "HK", "HH", "HHwithin"))[-c(1,6)]) +
  theme_minimal() + labs(y="Flowers per g biomass, one infloresence", x="Total inflorescence biomass (g)", color="Cross")
```
