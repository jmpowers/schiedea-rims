...
Appears linear after log-log transformation at first glance, but actually not! 
...
F1.RI <- F1.emm %>% separate(sxc, into=c("momsp","crosstype")) %>% 
  ...
  rowwise() %>% mutate(across(c(between,hybrid), ~ ./max(between,hybrid)),
                       RI = 1 - 2 *  hybrid/(hybrid + between)) %>% #method of Sobel and Chen 2014
...
F1.CRI.m <- F1.RI %>% filter(str_detect(sex,"m")) %>%  select(-c(step,between,hybrid,sex)) %>% 
...
F1.RI %>% select(-step) %>% pivot_wider(names_from=momsp, values_from=c(between, hybrid, RI)) %>% 
  left_join(F1.CRI.m) %>% 
...
```{r totalRI_simulation, fig.height=8, fig.width=8}
ntrials <- 10000

sample_trait <- function(dataname, yvar, N) {
  get(dataname) %>% select(sxc, y=!!yvar) %>% drop_na(y) %>% #drop missing trait values!
    nest(dfchunk=c(y)) %>%  mutate(sampled = map(dfchunk, slice_sample, n=N, replace=T)) %>% 
    select(-dfchunk) %>% unnest(sampled) %>% arrange(sxc) %>% mutate(trial=row_number())
}

set.seed(1)
F1.cumul.fitness <- traits %>% filter(generation=="F1", str_detect(sex,"m")) %>% select(data, yvar) %>% 
  mutate(samp = pmap(., sample_trait, N = ntrials)) %>% select(-data) %>% unnest(samp) %>% 
  pivot_wider(id_cols=c(sxc, trial), names_from=yvar, values_from=y) %>% 
  mutate(fitness_female = capsule.formed * viable.seeds * prop.germ * alive * 
           flowered * flower.number,
         fitness_male = capsule.formed * viable.seeds * prop.germ * alive * 
           flowered * flower.number * total.per.anther * prop.viable)

F1.cumul.fitness %>% select(sxc, starts_with("fitness")) %>% 
  pivot_longer(starts_with("fitness"), names_to="sex", names_prefix = "fitness_") %>% 
  ggplot(aes(x=value, color=sxc)) + facet_wrap(vars(sex), scales="free_x")+
  geom_freqpoly(bins=200, boundary=1, position=position_identity(), size=1) +
  ...
  labs(y=paste0("Frequency of outcome (",ntrials," trials)"), 
       x="FF1 flowers or F1 viable pollen grains resulting from one P x P pollination",
       title = "Distribution of female and male cumulative F1 fitness (if nonzero)") 

F1.cumul.fitness %>% group_by(sxc) %>% summarize(prop.zero=mean(fitness_female>0)) %>% 
ggplot(aes(y=prop.zero, x=sxc, color=sxc)) + 
  ...
  labs(y=paste0("Frequency of nonzero F1 flowers resulting from one P x P pollination (",ntrials," trials)"), x="Crosstype",
...
F1.inbred <- F1.emm %>% separate(sxc, into=c("momsp","crosstype")) %>% 
  ...
  select(trait, momsp, crosstype, estimate) %>% pivot_wider(names_from=crosstype, values_from=estimate) %>% 
  rowwise() %>% mutate(across(c(between,within), ~ ./max(between,within)),
                       ID = 1 - within/between) %>% 
    left_join(traits %>% filter(generation=="F1") %>% select(trait=yvar, step, sex)) %>% arrange(step) %>% 
      mutate(trait = recode(trait, !!!set_names(traits$ylab[-1], traits$yvar[-1])) %>% 
               str_remove(fixed(" (g)")) %>% factor() %>% fct_reorder(step)) 

F1.inbred %>% select(-step) %>% 
...
F1.CID.m <- F1.inbred %>% filter(str_detect(sex,"m")) %>%  select(-c(step,between,within,sex)) %>% 
...
F1.inbred %>% select(-step) %>% pivot_wider(names_from=momsp, values_from=c(between, within, ID)) %>% 
  left_join(F1.CID.m) %>% 
...
ggplot(F1.inbred, aes(y=fct_rev(trait), x=ID, color=momsp)) + 
  ...
  scale_x_continuous(limits=c(-1.1,1.1), breaks=c(-1,-0.5,0,0.5,1),
                     labels=c("-1\noutcrossing\ndepression","-0.5","0\nneutral","0.5","1\ninbreeding\ndepression")) + 
  theme_minimal() + theme(legend.position = "top", legend.text = element_text(face="italic"))+
  labs(x="Inbreeding depression", y="Fitness component", color="Species")
