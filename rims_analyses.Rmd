...
plot_F1 <- function(emm, data, yvar, ylab, type="continuous") {
  ...
  #TODO get this to work with binary models by back-transforming
  if(type=="continuous") {
    sig <- sig %>% mutate(prop = -estimate/emm, prop.se = std.error/emm)
...
F1.plots <- traits %>% filter(generation=="F1",yvar!="viable.per.anther") %>% select(data, yvar, ylab, type) %>% 
  pmap(plot_F1, emm=F1.emm)

gridExtra::grid.arrange(grobs=F1.plots, nrow=4, heights=c(1,1,1,1.22), ncol=3, as.table=F)
```
...
ggplot(F1.RI %>% filter(trait!="Viable pollen grains per anther"), aes(y=fct_rev(trait), x=RI, color=momsp)) + 
